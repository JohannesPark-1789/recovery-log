...
- **Phase** · 8단계 회복 타임라인 (현재 phase 자동 하이라이트 + 진행률 / 지나간 phase DONE 표시), **수술 세부항목 보기** (200+ 항목, 6개 그룹: 수술·마취·입원·약물·검사·보조기, 검색·필터·금액 표시), **Medical records** (경과기록지·수술기록지·진단서·입퇴원확인서·의무기록사본 5종 — 진단명·KCD·집도의·소견·6주 가료 일정 등 자동 시드), 수술부 사진 (D별 정렬 + **Compare 모드** 로 두 시점 side-by-side 비교) / X-ray / 진단서 기록, Fraunces serif 저널 (사진 첨부 가능)
- **Learn** · **Injury mechanics** (사고를 다운힐→마찰원→정지/운동마찰 전환→로우사이드→어깨 충격 5단계로 분해 + 체중·속도·낙하높이·접촉시간·μ_k 등 입력값을 바꾸면 충격속도·평균/피크 충격력·에너지·쇄골 하중이 실시간 재계산되는 force calculator, 골절 임계값 2 kN 대비 배율 시각화), **의학용어집 66개 항목** 12개 카테고리 (해부학·진단·수술·골절치유·세포·조직회복·통증·활력징후·재활·약리·의료기기·상처관리), 한/영/약어 검색 지원
- **Cost** · 사고 관련 전체 비용 추적 (진료비·약값·교통·보조용품·소모품·입원비·기타), 영수증 사진 첨부, 보험·산재·가족 환급 차감해 순 본인부담 계산, 카테고리별 비중 표시
- **Search** · 헤더 우상단 🔍 → 저널·통증·약·문서·X-ray·비용·수술항목·**의무기록**·용어집·vitals 전체 통합 검색, 결과 클릭 시 해당 탭으로 이동 + 항목 하이라이트
...
- **Patient:** *** (PT ***, RM 312, ***병원 정형외과)
- **Vitals D1:** ****-**-** 20:30 · SpO₂ 97%, PR 64bpm, RR 8, PI 10.6% (mobiCARE +Pulse)
